...
temp_col = "tmax" if temp_type == "TMAX" else "tmin"
...
c1.metric("Latest", f"{df[temp_col].iloc[-1]:.1f} °C")
c2.metric("Average", f"{df[temp_col].mean():.1f} °C")
c3.metric("Max", f"{df[temp_col].max():.1f} °C")
...
yearly = trend.groupby("year")[temp_col].mean().reset_index()

fig_trend = px.line(yearly, x="year", y=temp_col, title="Long-Term Temperature Trend")
...
# -----------------------------
# SAFE FORECAST METHOD
# -----------------------------
def forecast_temperature(series, days):
    """
    Robust forecasting using weighted moving average
    (never crashes)
    """
    preds = []
    history = list(series[-30:])  # last 30 days

    for _ in range(days):

        # weighted average of recent temps
        weights = np.linspace(1, 2, len(history))
        pred = np.average(history, weights=weights)
...
if st.sidebar.button("Generate Forecast"):
    ...
    preds = forecast_temperature(df[temp_col], forecast_days)
